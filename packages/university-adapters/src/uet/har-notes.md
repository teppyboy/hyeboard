...
**Auth model differs from the student flow**: parent accounts first request `GET /api/auth/captcha`, then submit `{ userName, password, captchaId, captchaValue }` to `POST /api/auth/login` — no Google OAuth popup or browser automation. The response includes an access token and account code plus account fields that Hyeboard discards. All subsequent calls use the same bearer-token pattern as the student flow once authenticated. The parent-facing notification endpoint uses an account code prefixed `PH...` (`GET /api/noti/user/PH{code}`) — likely "Phụ huynh" (Vietnamese for parent/guardian) — a distinct account-code namespace from student codes.
